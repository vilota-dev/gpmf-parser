...
GPMF_ERR readMP4File(char* filename)
{
	...
	if (metadatalength > 0.0)
	{
		...
		// Find all the available Streams and compute they sample rate
			double start, end;
			...
			double drate = GetGPMFSampleRate(cbobject, STR2FOURCC("ISOE"), STR2FOURCC("SHUT"), GPMF_SAMPLE_RATE_PRECISE, &start, &end);
			printf("  %c%c%c%c sampling rate = %fHz (time %f to %f)\",\n", PRINTF_4CC(fourcc), drate, start, end);
		}	
